...
function searchForMemes($userID = '') {
    ...
    else if($_POST['order']=="oldest-first") {
      $sql .= " ORDER BY create_date ASC";
    }
    
    
    $search = $_POST['search'];
    $mSearch = $_POST['meme-type-search'];
    $statement = $dbConn->prepare($sql); 
    $statement->execute(array(':search'=> '%'.$search.'%', ':mSearch' => $mSearch)); 
...
function createMeme($line1, $line2, $memeType) {
    ...
    //Step 2: Insert the meme information (along with the category ID) into the all_memes2 table
